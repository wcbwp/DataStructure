...
| :--- | :--- | :--- | :--- |
| 当前 `data` 地址（图 C 为 `v.data`） | 0x0000000001021920| 0x0000000001021920| 0x0000000001021920|
| `newData` 地址 | 尚未申请，不填写 | 0x0000000001021960 | 已离开作用域，不填写 |
| 当前对象的 `size` | 4 | 4 | 5 |
...
> 在此填写你的回答（不少于 20 字）。
不可以在程序中访问旧数组，因为delete[]会释放旧数组占用的堆内存，内存使用权交给操作系统。Memory View看到的只是内存中残留的旧数据，此时原指针变为野指针，在代码里访问属于未定义行为，肯能造成程序崩溃。
#### 3.6.5 截图检查标准
...
> 在此填写你的回答（不少于 30 字）。
扩容必须把旧数组里面的元素复制到新开辟的大容量数组，再释放旧数组；如果先释放就数组，原有数据会丢失，无法拷贝。size代表当前有效元素个数，capacity是总存储容量；扩容只是扩大存储空间，不会新增有效元素，不能直接把size设置为capacity
---
