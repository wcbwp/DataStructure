...
| :--- | :--- | :--- | :--- |
| 当前 `data` 地址（图 C 为 `v.data`） | 0xe817f0 | 0xe817f0 | 0xe81830 |
| `newData` 地址 | 尚未申请，不填写 | 0xe81830 | 已离开作用域，不填写 |
| 当前对象的 `size` | 4 | 4 | 5 |
...
> 不能。`delete[]` 释放后数组内存已归还，旧地址失效，继续访问属于未定义行为，可能读到残留数据或导致程序崩溃；释放后应通过 `data` 访问新数组。
...
> 因为必须先让新数组复制好有效元素，旧数组才不再需要；若先释放旧数组则旧元素丢失、无法完成复制。扩容只是预留更多空间，size 表示有效元素个数，直接改成 capacity 会把尚未写入的空位当作有效元素。
