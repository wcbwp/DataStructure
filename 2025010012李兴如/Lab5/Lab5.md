...
| :--- |:-------------------|:-------------------|:---------------------|
| 当前 `data` 地址（图 C 为 `v.data`） | 0x0000000000fc1920 | 0x0000000000fc1920 | 0x0000000000fc1920   |
| `newData` 地址 | 尚未申请，不填写   | 0x0000000000fc1920 | 已离开作用域，不填写 |
| 当前对象的 `size` | 4                  | 4                  | 8                    |
...
> 不可以，虽然内存视图还能看见残留数据，但指针在此时已经变成了野指针，访问会失败。
...
- [x] B. 每一次 `pushBack` 都必须复制全部元素，所以都是 `O(n)`
...
> 如果先释放旧数组，会导致数据丢失；如果直接把"size"设成"capacity“，会导致数组内部数据的数量及容量变混乱，致使结果错误。
