...
> insertAt 从后向前移动可以避免尚未移动的元素被覆盖（每个元素移到下一个位置时，它的原位置还没有被写入新值）；removeAt 从前向后移动同样避免覆盖（每个元素移到上一个位置时，它的原位置已经被复制走了）。如果方向反了，会导致元素被重复覆盖，数据丢失，最终顺序表中的数据错误。
...
- 自动审核只检查是否完成，不判断答案和程序输出和文字回答是否正确，内容正确性将在期末统一检测。
- 不需要提交运行截图。
