...
| C（Lab3 / 课堂） | C++（本次作业） |
| :--- | :--- |
| `SeqList list; list.size = 0;` | IntVector v;（在此填写） |
| `insertAt(&list, 2, 25)` | v.insert(2, 25)（在此填写） |
| `removeAt(&list, 1, &removed)` | v.remove(1, removed)（在此填写） |
| 函数体内的 `list->data[i]` | data[i]（在此填写） |
| 返回 `int` 的 `1` / `0` | 返回 `bool` 的 `true` / `false`（在此填写） |

### 4.13 填空：一句话总结

Lab3 的 `main` 里可以直接写 `list.size = 4;`。用一句话说明：为什么把 `size` 设为 `private` 之后，`insert`、`remove` 里的边界检查才真正"管用"？

将 size 设置为 private 后，外部代码不能直接修改 size，只能通过 insert、remove 成员函数修改，所有修改都会经过函数中的边界检查，防止外部随意篡改 size 破坏顺序表。> 在此填写你的回答（不少于 20 字）。
