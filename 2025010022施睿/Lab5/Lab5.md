...
| :--- | :--- | :--- | :--- |
| 当前 `data` 地址（图 C 为 `v.data`） | `0x000000000010a1810` | `0x000000000010a1810` | `0x000000000010a1850` |
| `newData` 地址 | 尚未申请，不填写 | `0x000000000010a1850` | 已离开作用域，不填写 |
| 当前对象的 `size` | 4 | 4 | 5 |
...
- `sizeof(int)`：4 字节。
- 容量为 `4` 时，整数元素存储区大小：16 字节（`capacity × sizeof(int)`）。
- 容量为 `8` 时，整数元素存储区大小：32 字节（`capacity × sizeof(int)`）。

如果 Memory View 在 `delete[]` 之后仍显示旧数组的字节，是否可以继续在程序中访问旧数组？说明理由。

> 不可以。`delete[]` 之后，这块内存已经交还给内存管理（allocator），`data` 变成悬空指针，继续访问属于未定义行为：虽然 Memory View 里可能还残留旧的字节没有被系统清零，但这段内存可能已经被分配给其他对象或随时被改写，读取它既不能得到可靠数据，也可能导致程序崩溃或破坏其他数据。所以程序中必须在释放之后立即让指针指向新的有效数组，绝不能再读旧数组。
...
> 因为旧数组是唯一保存有效元素的来源，必须先把它复制到新数组，否则一旦 `delete[] data` 释放旧数组，旧数据就全部丢失，程序无法再读取。扩容只是"腾出空位"，有效元素的个数并没有变，`size` 必须继续表示"真正存了多少个元素"，而 `capacity` 只表示"最多能存多少个"；如果直接把 `size` 设成 `capacity`，等于把大量空的位置当成了有效元素，之后的遍历、输出、查找都会出错。
