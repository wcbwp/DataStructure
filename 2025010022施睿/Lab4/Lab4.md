...
| C（Lab3 / 课堂） | C++（本次作业） |
| :--- | :--- |
| `SeqList list; list.size = 0;` | `IntVector v;`（构造函数自动把 `size` 设为 `0`） |
| `insertAt(&list, 2, 25)` | `v.insert(2, 25)` |
...
> 因为 `size` 是 `private` 的，类外任何代码都无法绕过边界检查直接修改它，`size` 只能通过 `insert`、`remove` 等带检查的成员函数变化，这样 `size` 就始终合法，边界检查才能真正保证表的每个操作安全。
