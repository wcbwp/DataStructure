...
| :--- | :--- | :--- | :--- |
| 当前 `data` 地址（图 C 为 `v.data`） | 0x0000024B6A50F5A0 | 0x0000024B6A50F5A0 | 0x0000024B6A50F5C0 |
| `newData` 地址 | 尚未申请，不填写 | 0x0000024B6A50F5C0 | 已离开作用域，不填写 |
| 当前对象的 `size` | 4 | 4 | 5 |
...
> 不可以。因为 `delete[]` 已经释放了旧数组的内存，这块内存可能已被其他程序或操作系统重新分配使用。在程序中继续访问旧数组会导致未定义行为，可能读取到无效数据或引发程序崩溃。释放后的指针虽然可能仍显示旧地址，但该地址指向的内存已不再属于当前程序。
...
> 扩容时必须先复制再释放旧数组，因为如果先释放旧数组，旧数组中的数据就会丢失，无法复制到新数组中。扩容后不能直接把 `size` 设成 `capacity`，因为 `size` 表示有效元素个数，`capacity` 表示数组总容量，两者含义不同。扩容只是增加了可用空间，并没有增加有效元素，所以 `size` 应保持不变。
