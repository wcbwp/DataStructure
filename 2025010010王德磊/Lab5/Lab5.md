...
> 不可以继续在程序中访问旧数组。因为 `delete[]` 执行后，旧数组所占的内存已经被释放归还给操作系统，这块内存可能已被其他程序或同一程序的其他部分重新分配和使用。虽然调试器可能仍显示残留的字节数据，但这些数据已经无效，继续访问属于未定义行为，可能导致程序崩溃、数据损坏或其他不可预测的错误。指针 `data` 在 `expand()` 中已被更新为新数组的地址，旧地址不再受对象管理。
...
> 扩容时必须先复制再释放旧数组，因为一旦释放旧数组，其内存可能被系统回收或重用，此时再通过旧指针访问其中的数据会导致未定义行为（如程序崩溃或数据损坏）。只有先将有效元素复制到新数组，才能安全释放旧数组。扩容后不能直接把 `size` 设成 `capacity`，因为 `size` 表示当前已存入的有效元素个数，而新申请的空间只是备用空位，尚未写入任何新元素；若直接把 `size` 设为 `capacity`，会导致后续操作误认为这些空位已有有效数据，从而产生逻辑错误。
