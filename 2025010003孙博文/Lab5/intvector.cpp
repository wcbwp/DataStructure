...
private:
    ...
    void expand() {
        // 若 size < capacity，还有空位，立即返回
        if (size < capacity) {
            return;
        }
        // 计算 newCapacity = capacity * 2
        int newCapacity = capacity * 2;
        // 用临时指针 newData 接收 new int[newCapacity]
        int* newData = new int[newCapacity];
        // 将下标 0 到 size - 1 的有效元素逐个复制到新数组的相同下标
        for (int i = 0; i < size; i++) {
            newData[i] = data[i];
        }
        // 释放旧数组
        delete[] data;
        // 让 data 指向新数组，再更新 capacity，保持 size 不变
        data = newData;
...
public:
    explicit IntVector(int initialCapacity = 4) {
        // 先把小于 1 的 initialCapacity 调整为 1
        if (initialCapacity < 1) {
            initialCapacity = 1;
        }
        // 申请对应容量的数组
        data = new int[initialCapacity];
        // 设置 size = 0 和 capacity = initialCapacity
        size = 0;
