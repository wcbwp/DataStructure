...
| :--- | :--- | :--- | :--- |
| 当前 `data` 地址（图 C 为 `v.data`） | 0x0000000000df17f0 | 0x0000000000f11830 | 0x0000000000fc17f0 |
| `newData` 地址 | 尚未申请，不填写 | 0x0000000000fc17f0 | 已离开作用域，不填写 |
| 当前对象的 `size` | 4 | 4 | 5 |
| 当前对象的 `capacity` | 4 | 8 | 5 |
...
> 在此填写你的回答（不少于 20 字）。
> 不可以继续访问，因为操作之后旧数据的内存已经被操作系统回收，再次访问属于非法操作，会导致程序奔溃。
