...
| :--- | :--- | :--- | :--- |
| 当前 `data` 地址（图 C 为 `v.data`） | 0x000001e0a34f56a0 | 0x000001e0a34f56a0 | 0x000001e0a34f5710 |
| `newData` 地址 | 尚未申请，不填写 | 0x000001e0a34f5710 | 已离开作用域，不填写 |
| 当前对象的 `size` | 4 | 4 | 5 |
...
> 不可以继续访问旧数组。虽然Memory View可能仍显示旧地址的字节内容，但这些数据已经是无效的，因为delete[]已经释放了该内存区域。继续访问已释放的内存属于未定义行为，可能导致程序崩溃或数据错误。
...
> 扩容时必须先复制再释放旧数组，因为旧数组是唯一保存有效数据的地方，如果先释放就无法复制了，会导致数据丢失。扩容后不能直接把size设成capacity，因为size表示实际存入的有效元素个数，扩容只是增加了可用空间，并没有自动添加新元素，所以size应保持不变，等待后续插入操作逐步填充。
