...
> 把 size 设为 private 之后，类外的任何代码（包括 main 函数）都无法直接修改 size，只能通过 insert 和 remove 等成员函数来操作。而这些成员函数在修改 size 之前都会先进行边界检查，从而保证 size 始终反映表中实际有效元素的个数，避免了非法操作导致数据不一致的问题。
