...
| :--- |:-----------------|:---------------|:---------------------|
| 当前 `data` 地址（图 C 为 `v.data`） | 0x6917d0         | 0x691810       | 0x691810             |
| `newData` 地址 | 尚未申请，不填写 | 0x691810       | 已离开作用域，不填写 |
| 当前对象的 `size` | 4                | 4              | 5                    |
...
> 不可以。delete[] 执行后，旧数组所在的内存已经被释放，操作系统可能将这块内存分配给其他用途。虽然 Memory View 可能仍残留旧字节，但程序访问已释放的内存属于未定义行为，可能导致数据错乱或崩溃，因此绝不能继续访问旧数组。
...
> 扩容时必须先复制再释放旧数组，因为旧数组是唯一保存有效数据的地方，一旦先释放，数据就丢失了，无法复制到新数组。扩容后不能把 size 设成 capacity，因为 size 表示实际存了多少个有效元素，capacity 表示数组总容量，两者含义不同；扩容只是提供了更多空位，还没有写入新元素，所以 size 应保持不变。
