...
> insertAt 中从后向前移动，能保证每个元素在覆盖前已被复制到后一个位置，后一个元素不会丢失；如果反方向移动先从前往后，后面的元素还没转移就会被前面的元素覆盖丢失。removeAt 中从前向后移动，后一个元素正好依次填入前一个空出的位置；如果反方向移动，前面的元素会覆盖尚未归位的相邻元素，同样造成数据丢失。因此移动方向必须与元素填补空位的方向一致。
