...
| :--- |:-----------------|:---------------|:---------------------|
| 当前 `data` 地址（图 C 为 `v.data`） | fc17f0           |   fc17f0             |   fc17f0                   |
| `newData` 地址 | 尚未申请，不填写 | 10a1830        | 已离开作用域，不填写 |
| 当前对象的 `size` | 4                | 4              | 8                    |
...
> 在此填写你的回答（不少于 20 字）。
不可以。delete[]把这块堆内存归还给了操作系统，程序丢失这块内存的合法所有权。虽然内存视图还能看到残留数据，但是这块内存随时会被其他代码覆盖，访问属于未定义行为，会造成程序崩溃，数据错乱。
#### 3.6.5 截图检查标准
...
> 在此填写你的回答（不少于 30 字）。
扩容需要先开辟新内存复制旧数组元素，再释放旧数组；如果先释放旧数组，原有数据会丢失。size记录的是容器中有效元素个数，capacity是最大容纳量，二者含义不同，不能直接等同赋值
---
