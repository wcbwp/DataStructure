...
| C（Lab3 / 课堂） | C++（本次作业） |
| :--- | :--- |
| `SeqList list; list.size = 0;` | `IntVector v;`（构造函数自动把 `size` 置为 `0`） |
| `insertAt(&list, 2, 25)` | `v.insert(2, 25)` |
| `removeAt(&list, 1, &removed)` | `v.remove(1, removed)` |
| 函数体内的 `list->data[i]` | 函数体内的 `data[i]` |
...
> 把 `size` 设为 `private` 后，类外的代码无法绕过 `insert`、`remove` 等成员函数直接修改 `size`，表的状态只能通过这些带边界检查的操作来更新，因此 `size` 永远与数组中实际的有效元素个数一致，边界检查才能真正生效。
