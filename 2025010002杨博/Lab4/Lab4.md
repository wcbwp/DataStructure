...
| C（Lab3 / 课堂） | C++（本次作业） |
| :--- | :--- |
| `SeqList list; list.size = 0;` | `IntVector v;` （构造函数自动将size设为0） |
| `insertAt(&list, 2, 25)` | `v.insert(2, 25)` |
| `removeAt(&list, 1, &removed)` | `v.remove(1, removed)` |
| 函数体内的 `list->data[i]` | `data[i]` （成员函数内直接访问） |
...
> 因为`size`设为`private`后，类外代码无法直接修改它，只能通过成员函数操作，这样`insert`和`remove`中的边界检查才能确保`size`的修改是安全的，防止非法操作导致数据不一致或程序崩溃。
