...
（注意：下面四个标为“截图后填入”的地址格必须替换为自己截图里真实看到的十六进制地址后，才能提交。）

| 记录项目 | 图 A：扩容前 | 图 B：复制完成 | 图 C：追加完成 |
| :--- | :--- | :--- | :--- |
| 当前 `data` 地址（图 C 为 `v.data`） | `0xf61960` | `0xf61960` | `0xf619a0` |
| `newData` 地址 | 尚未申请，不填写 | `0xf619a0` | 已离开作用域，不填写 |
| 当前对象的 `size` | 4 | 4 | 5 |
...
> 不可以。`delete[]` 已经把这块数组的内存归还给系统，`data`（旧数组地址）成为悬空指针，继续访问是未定义行为：虽然调试器可能残留显示旧字节，但程序再次读写可能读到随机数据或直接崩溃，所以释放后不能再访问旧数组。
...
> 如果先释放旧数组再复制，旧数据在释放时就已经丢失，所以必须先申请新数组、把有效元素复制过去，再释放旧数组、更新指针和容量。`size` 表示已经存放的有效元素个数，扩容只改变容量、并没有新增任何元素，所以 `size` 应保持不变，不能直接设成 `capacity`。
