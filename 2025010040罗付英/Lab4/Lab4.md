...
| C（Lab3 / 课堂） | C++（本次作业）                    |
| :--- |:-----------------------------------|
| `SeqList list; list.size = 0;` | （在此填写）IntVector v;           |
| `insertAt(&list, 2, 25)` | （在此填写）v.insert(2,25);        |
| `removeAt(&list, 1, &removed)` | （在此填写）remove(1,removed);     |
| 函数体内的 `list->data[i]` | （在此填写）data[i]                |
| 返回 `int` 的 `1` / `0` | （在此填写）返回bool的true / false |
...
> 在此填写你的回答（不少于 20 字）。
将size设为private后，外部代码无法直接修改size，只能通过insert、remove成员函数修改，只有经过边界检查的代码才能改变size的值。
---
